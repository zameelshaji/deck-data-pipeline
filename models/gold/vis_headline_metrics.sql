...
cumulative_swipes as (
    select
        ds.metric_date,
        count(case when e.event_type in ('swipe_right', 'swipe_left') then 1 end) as total_swipes,
        count(case when e.event_type = 'swipe_right' then 1 end) as total_swipes_right,
        count(case when e.event_type = 'swipe_left' then 1 end) as total_swipes_left,
        count(distinct case when e.event_type in ('swipe_right', 'swipe_left') then e.user_id end) as users_who_swiped
    from date_spine ds
    left join {{ ref('stg_unified_events') }} e
...
daily_new_metrics as (
    ...
    left join {{ ref('stg_unified_events') }} e on e.event_timestamp::date = ds.metric_date
    group by ds.metric_date
)
