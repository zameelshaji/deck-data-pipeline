...
weekly_metrics as (
    select
        ...
        count(distinct case when source_table = 'dextr_queries' then user_id end) as wau_ai_users,
        count(distinct case when event_category in ('Swipe', 'Save') then user_id end) as wau_curation_users,
        count(distinct case when event_category = 'Conversion' then user_id end) as wau_conversion_users,
        count(distinct case when source_table = 'featured_section_actions' then user_id end) as wau_featured_users,
