...
with user_journey as (
    select 
        ...
        min(case when e.event_source = 'dextr' then e.event_timestamp end) as first_ai_query,
        min(case when e.event_category = 'Content Curation' then e.event_timestamp end) as first_curation,
        min(case when e.event_category = 'Conversion Action' then e.event_timestamp end) as first_conversion,
        
        -- Activity counts in first 7 days
        count(case 
        ...
        -- Distinct days active in first week
        count(distinct case 
            when e.event_timestamp between u.created_at and u.created_at + interval '7 days' 
...
cohort_summary as (
    select 
        ...
        -- First actions (within any timeframe)
...
select 
    *,
    ...
    -- Rolling 7-day totals for smoother trends
    sum(signups) over (
