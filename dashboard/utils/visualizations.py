...
# ============================================
# NORTH STAR METRIC VISUALIZATIONS
# ============================================

def create_north_star_funnel_chart(stages, values, title="Session Funnel"):
    """Create horizontal funnel showing drop-off at each stage for North Star metrics"""
    # Pink gradient from dark to light
    gradient_colors = ['#E91E8C', '#FF2E9C', '#FF4FA3', '#FF6BB5', '#FF8DC7', '#FFAED7', '#FFD0E8']
    ...
    fig = go.Figure(go.Funnel(
        ...
        textinfo="value+percent initial",
        textposition="inside",
        textfont=dict(size=11, color='white', family='Inter, -apple-system, sans-serif'),
        marker=dict(
            color=gradient_colors[:len(stages)],
...
def create_kpi_trend_chart(df, x_col, kpi_cols, title, y_label="Percentage"):
    """Create multi-line trend chart for North Star KPIs"""
    fig = go.Figure()
    ...
    for i, col in enumerate(kpi_cols):
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name=col.replace('_', ' ').title(),
            line=dict(color=CHART_COLOR_SEQUENCE[i % len(CHART_COLOR_SEQUENCE)], width=2),
            marker=dict(size=6, color=CHART_COLOR_SEQUENCE[i % len(CHART_COLOR_SEQUENCE)])
        ))
...
def create_surface_comparison_chart(df, x_col, y_col, color_col, title):
    """Create grouped bar chart comparing metrics across surfaces"""
    fig = px.bar(
        df,
        x=x_col,
        y=y_col,
...
def create_activation_heatmap(df, x_col, y_col, value_col, title="Activation Heatmap"):
    """Create heatmap for activation cohort analysis"""
    import pandas as pd
    ...
    # Format labels
    y_labels = [pd.to_datetime(idx).strftime('%b %d') for idx in pivot_df.index]
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        hovertemplate='Cohort: %{y}<br>Metric: %{x}<br>Value: %{z:.1f}%<extra></extra>'
    ))
...
def create_save_source_pie(df, title="Save Sources"):
    """Create pie chart for save source breakdown"""
    fig = px.pie(
        df,
        values='save_count',
        names='save_source',
        title=title,
        color_discrete_sequence=['#E91E8C', '#FF6BB5', '#FFB3D9']
    )
...
def create_ttfs_histogram(df, col='ttfs_seconds', title="Time to First Save Distribution"):
    """Create histogram for TTFS distribution"""
    fig = px.histogram(
        df,
        x=col,
        nbins=30,
    ...
    fig.update_traces(marker_color='#E91E8C', marker_line_width=0)
